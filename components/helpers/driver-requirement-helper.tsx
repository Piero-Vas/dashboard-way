import { ActionModalDialog, StatusDriverRequirement, StatusRegisterDriver } from "@/lib/enums";
...
import { getIconModalDialog, getStatusClass, getStatusDriverRequirement } from "@/lib/utils";
import { UploadCloud } from "lucide-react";
import { useState } from "react";
import { handleUpdate } from "@/hooks/use-fetch-driver-requirement";
import { DriversRequests } from "@/types/driver-requests";

export const ModalDialog = ({ item, action, disabled }:
  { item: DriversRequests, action: ActionModalDialog, disabled: boolean }) => {
...
export const getActionsForStatus = (
  ...
  handleStatusUpdate: (id: number, item: DriverRequirement, status: StatusRegisterDriver) => void,
  ...
  if (status === StatusRegisterDriver.PENDING) {
    actions.push(
      <DropdownMenuItem
        key="approve"
        onClick={() => handleStatusUpdate(item.id, item, StatusRegisterDriver.ACTIVE)}
      >
    ...
    actions.push(
      <DropdownMenuItem
        key="reject"
        onClick={() => handleStatusUpdate(item.id, item, StatusRegisterDriver.REJECTED)}
      >
